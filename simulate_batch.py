...
def request_generator(env, arrival_rate, server):
    ...
    while True:
        ...
        request_id += 1
        # Add request to timeline as soon as it's generated
        request_timeline.append({
            'Request': request_id,
            'Start': None,  # Will be filled when request starts processing
            'End': None,    # Will be filled when request ends processing
            'Wait': None,   # Will be filled when request starts processing
            'Processing': None, # Will be filled when request ends processing
            'BatchSize': None,  # Will be filled during processing
            'Node': None,       # Will be filled during processing
            'Generated': env.now # New field to track generation time
        })
...
def handle_request(env, server, request_id):
    ...
    with server.request() as request:
        ...
        # Update the existing request entry in the timeline
...
def plot_metrics():
    ...
    for _, row in timeline_df.iterrows():
        if row['Start'] is not None:  # Only plot processed requests
